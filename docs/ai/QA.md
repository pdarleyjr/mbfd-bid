...
- BLOCKED (annual data readiness, no mutation) — authorized local-source search found no current raw official TeleStaff Assignments HTML export. The available May Excel XML export and 2026 vacation XML cannot pass the fail-closed HTML parser and were not converted or imported. Consequently there is no reviewable official assignment import or accepted 2026 baseline from which to construct a mock roster.
- BLOCKED (policy designation, no mutation) — the checked-in 2026 Rules & Points explicitly says Fire Investigator inclusion requires admin confirmation; the position template leaves the Station 6 Marine FF/Post St.6 dashboard classification open. The documents therefore do not independently authorize a staging-only annual configuration designation. No rule book, configuration, or mock was created.
- PASS (read-only serving-version check) — API version inventory still ends at the post-rotation secret-only version `6d8ef44a-27e3-4399-9fb6-f844e6868cb6`; no secret rotation occurred in the annual-readiness assessment.
