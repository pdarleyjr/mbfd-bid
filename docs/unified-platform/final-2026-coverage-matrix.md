...
personnel may be forced**. The later continuation separately confirms the AirTech-specific minimum-qualified
fallback and the designated FF/DE A-Day scope after fresh final-source verification.
...
| Prevention and Events Captain/Lieutenant; PDF pp1-2 Procedure3(a-d) | Source-backed binary cumulative groups count each satisfied criterion once; no invented unequal weights. Shared schema, evaluator, editor and compiler golden tests cover Events four criteria and RN8977 I AND II as one criterion. | Missing arbitrary numeric weights are resolved as qualitative semantics. Current FL validity and exact credential catalog/qualifier evidence remain required. The old inspector-only Events table is intentionally not used as parity. |
| Investigator FF: State Fire Investigator AND FL Fire Inspector, IAAI-CFI tier, conditional secondary scoring and current-assignee fallback; PDF p2 Procedure3(e) | Both minimums are golden-tested. Ordered qualification metadata precedes cumulative credits; source-backed exclusion suppresses all three secondary credits for IAAI holders. Ordered comparison and canonical interruption guards now have passing focused synthetic verification, including non-IAAI-only secondary credits. | Generated draft explicitly represents the IAAI criterion and non-CFI-only credits. Final source305 maps semantically to old303 and keeps an explicit Ladder3 assignment exception. Actual qualification evidence and approved Department binding remain required. |
...
| Main AirTech FF/DE: Scott AND Cylinder/FSO AND DE; distinct backup preferences; PDF p4 Procedure7(a),(ii) | Compiled golden cases require both main certificates and DE, keep carseat optional, enforce all-six Operations and count backup Scott/Cylinder as binary preferences. Specific fallback is MINIMUM_QUALIFIED with reverse source-certified ordering. | Interpretation is closed by final PDF and the user continuation. Main and backup are independent. Workbook CY5 OR and B80 mandatory-carseat placement are intentional divergences. MASTER has no separately named backup seat, so backup opportunity binding remains a narrow source/configuration decision. |
...
| Station1/3 station-level FF/DE pools, Investigator exception; PDF p7 Procedure13(a-d) | Explicit station pools retain concrete capacity while ignoring daily apparatus labels; validate equal rules and reservation order. Canonical changed-award qualification and term gates are locally tested. | Source memberships and verified staffing bindings are required. Investigator stays a dedicated exception; do not include its seat in an interchangeable ordinary pool. Daily unit selection and final Department transition require separate acceptance. |
...
| Captain/Lieutenant/Firefighter order; PDF p8 Bid Selection4-5, footnote4 | Per-stage source-certified ordinal domains and versioned authority retain historical ordering separately. User continuation authorizes Rank Seniority to TIME_IN_GRADE_BID_ORDINAL and Straight Seniority to DEPARTMENT_SERVICE_BID_ORDINAL. | Semantic mapping is resolved. Actual identity reconciliation and imported frozen values remain separate evidence; unmatched, absent or tied values fail closed. No hire/promotion dates or personnel lifecycle values are manufactured. |
...
| MASTER topology; Positions!A1:I229 | Fresh extraction confirms228 positions, A74/B73/C73/D8. Deterministic comparison with captured old242 classifies133 exact continuations,47 material changes,34 new and14 narrow ambiguous final rows. Thirty-two continuations have changed source numbers. | No blanket human review of228 is required. The14 ambiguities are12 indistinguishable old-seat pair mappings plus B214/C214 source division contradiction. Ninety-six old identities are retired from the proposed topology; this is not a personnel mutation. No approved usable staffing bindings were established; all remain unbound. |
...
| Credential qualifiers and catalog equivalence | Verify validity, provider, passing status, endorsements and exact equivalents. Qualitative cumulative preferences now use binary credits and ordered criteria, not missing-number defaults. | Affected member eligibility/ranking; no alias or current-validity inference. |
...
## September19 continuation evidence

[The continuation source report](final-2026-continuation-source-review.md) records
the fresh input verification, narrowed decisions, semantic crosswalk and generated
canonical draft. The draft includes228 source profiles,223 biddable compiled rules and24 homogeneous
opportunity pools,10 qualified fallback policies,2 term groups,5 pending stage definitions
and3 source-scoped A-Day constraints. It keeps operational authority empty and
unknown numeric values null in `pendingPolicy`; strict execution policy remains
unconfigured. Schema validation, canonical normalization and223 decoder checks pass.

The generated artifact passes86 synthetic source-rule assertions. Separate
focused schema/evaluator/editor/source-golden suites are recorded in that report.
These are neither proof of actual member eligibility nor permission to create,
start, publish or apply a Real Bid. Open source/data decisions use explicit
`BLOCKS_REAL_BID_ACTIVATION` scope and do not alone block application release.
