...
- Latest local checkpoints: lint passed for 810 files; the web suite passed 260 tests before the final mobile-focus change, which passed focused unit and browser checks. Worker passed 1,206 tests with one explicit skip, plus all 25 launcher checks. Shared passed 166, A-Day 62, and eligibility 89 with the requested-evidence historical replay explicitly skipped. Typechecking passed after the current projection/mobile changes. These counts are checkpoints, not the final exact-candidate gate.
- Linux OpenNext staging artifact builds passed from the 21:05 and 21:45 source copies, using the same fixed staging API base as CI and making no deployment. The 21:45 copy's 954 source-file hashes were verified in Linux before building. Native Windows bundling failed on Sharp's native module. Later staffing refresh and mobile-focus changes are not included in that artifact; final artifact identity must be refreshed.
- Replay evidence now accepts an explicit private fixture directory, checks expected priority groups without inventing tie-breaks, and requires nonzero ordering coverage in an explicitly enabled historical run. Harness tests are synthetic. No private historical fixture manifest has been accepted or replayed.
