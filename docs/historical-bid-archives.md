...
Keep personnel data, source images, and generated archives outside the public Git repository. The application stores archives in the existing private exports bucket under `historical-bids/v1/<year>.json`, with publisher, publication time, and SHA-256. Conditional creation prevents replacement, including concurrent attempts. Exact retries are idempotent. Reads verify content integrity and year. There is no replace or delete operation.
