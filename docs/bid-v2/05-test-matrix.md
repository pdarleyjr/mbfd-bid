...
| Division Chief / POL-015 policy path | Source-local targeted run: 11 Worker files / 74 tests passed. It covers migration 0023 constraints, clone → draft-only participation → targeted rule deletion → coverage/diff → publication, authoritative-assignment pool exclusion, vacancy behavior, frozen-session behavior, eligibility, board preview, rehearsal, REST controls, and DO guards. | Complete the root local equivalent and clean Linux OpenNext gate for the final SHA. Staging still requires migration/recovery proof, approved actual staffing bindings, audited candidate lifecycle, and a safely isolated acceptance path; no staging rule data has been changed. |
