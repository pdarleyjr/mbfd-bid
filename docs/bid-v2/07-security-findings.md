...
| Obsolete Pages build adapter | BLOCKED | `@cloudflare/next-on-pages` is deprecated and its current peer range does not support the repository's already-locked Next version. Updating Next to the patched version for `sharp` would deepen that incompatibility. A staged OpenNext/Workers migration is a separate, authorized release workstream—not a package override. |
