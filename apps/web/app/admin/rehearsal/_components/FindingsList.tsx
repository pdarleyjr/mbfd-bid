...
export function FindingsList({ findings }: Props): ReactElement {
  if (findings.length === 0) {
    return (
      <p className="rounded border border-border bg-background p-4 text-sm text-muted-foreground">
        ...
        >
          <div className="flex flex-wrap items-baseline gap-2 text-xs text-muted-foreground">
            ...
            {f.screenshotR2Key !== null ? (
              <span className="font-mono text-info">{f.screenshotR2Key}</span>
            ) : null}
          </div>
          <p className="mt-1 whitespace-pre-wrap text-foreground">{f.note}</p>
        </li>
      ))}
    </ul>
  );
