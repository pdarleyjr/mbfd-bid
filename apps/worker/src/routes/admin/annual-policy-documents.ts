...
router.post('/:year', requireStepUpAuth(), zValidator('json', DraftDocumentSchema), async (c) => {
  ...
  const db = getDb(c.env.DB);
  const [bidYear, ruleBook, revisionRow, previousPublished] = await Promise.all([
    ...
    db
      .select({ revision: max(annualBidPolicyDocuments.revision) })
      .from(annualBidPolicyDocuments)
      .where(eq(annualBidPolicyDocuments.ruleBookVersion, body.rule_book_version))
      .get(),
    db
  ...
  const result = await c.env.DB.batch([
    ...
    auditInsertStatement(c.env.DB, {
      ...
      afterState: {
        ...
        rule_book_version: body.rule_book_version,
        revision,
      },
      reason: body.reason,
    }),
  ...
  return c.json({ id, revision, status: 'DRAFT' }, 201);
});
