...
describe('feature flag mutes AI endpoints', () => {
  let harness: TestD1;
  let env: WorkerEnv;
  ...
  it('advise-current returns 503 with feature_flag_off', async () => {
    ...
    expect(((await res.json()) as { reason: string }).reason).toBe('feature_flag_off');
  });

  it('advise-deep returns 503 with feature_flag_off', async () => {
    const jwt = await adminJwt(env);
    const res = await mkApp().request(
